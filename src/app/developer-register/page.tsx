...
export default function DeveloperRegisterPage() {
  ...
  if (status === 'none') return (
    <div style={{ maxWidth: 500, margin: '40px auto', padding: '0 20px' }}>
      <h1 style={{ fontSize: 24, fontWeight: 'bold', marginBottom: 20 }}>💻 开发者入驻申�?/h1>
  ...
  const handlePublish = () => {
    if (!agentName || !capability) return alert('请填写智能体名称和能�?);
    alert('智能体已提交审核�?);
    setAgentName('');
    setCapability('');
  };
  ...
  return (
    <div style={{ padding: '40px 20px', maxWidth: 900, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, fontWeight: 'bold', marginBottom: 8 }}>💻 开发者平�?/h1>
      <p style={{ color: 'var(--text-muted)', marginBottom: 24 }}>已签�?· 发布智能体，�?API 调用量获得收益�?/p>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '2px solid var(--border-color)' }}>
        {(['agents', 'revenue', 'withdraw'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {tab === 'agents' && '🤖 我的智能�?} {tab === 'revenue' && '💰 收益'} {tab === 'withdraw' && '💳 提现'}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'agents' && (
        <div style={{ background: 'var(--bg-card)', padding: 24, borderRadius: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <input placeholder="智能体名�? value={agentName} onChange={e => setAgentName(e.target.value)} style={inputStyle} />
        ...
        <div style={{ background: 'var(--bg-card)', padding: 24, borderRadius: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16 }}>
            ...
            <div style={{ background: '#f0fdf4', padding: 20, borderRadius: 10, textAlign: 'center' }}>
              <p style={{ color: 'var(--text-muted)' }}>API 调用�?/p>
