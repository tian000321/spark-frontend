...
export default function CreatorPage() {
  ...
  if (status === 'none') return (
    <div style={{ maxWidth: 500, margin: '40px auto', padding: '0 20px' }}>
      <h1 style={{ fontSize: 24, fontWeight: 'bold', marginBottom: 20 }}>🎨 创作者入驻申�?/h1>
    ...
    <div style={{ maxWidth: 600, margin: '0 auto', padding: 40 }}>
      ...
      <div style={{ background: '#f9fafb', padding: 20, borderRadius: 8, marginBottom: 20, maxHeight: 300, overflow: 'auto', fontSize: 13, lineHeight: 1.8 }}>
        ...
        <ol>
          ...
          <li>收益分账比例：创作�?70%，平�?30%（前 10 个氛围包创作者得 100%）�?/li>
          <li>平台有权对违规行为进行处理�?/li>
          <li>本协议解释权归星火科技所有�?/li>
        </ol>
  ...
  return (
    <div style={{ padding: '40px 20px', maxWidth: 900, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, fontWeight: 'bold', marginBottom: 8 }}>🎨 创作者平�?/h1>
      <p style={{ color: 'var(--text-muted)', marginBottom: 24 }}>已签�?· 上传氛围包，审核通过后进入市场�?/p>
      ...
      {activeTab === 'upload' && (
        <div style={{ background: 'var(--bg-card)', padding: 24, borderRadius: 12 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <input placeholder="氛围包名�? value={packageName} onChange={e => setPackageName(e.target.value)} style={inputStyle} />
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              ...
              <select value={licenseType} onChange={e => setLicenseType(e.target.value)} style={inputStyle}>
                <option value="exclusive">独家授权</option><option value="non-exclusive">非独�?/option><option value="cc">CC</option>
              </select>
            </div>
            <div style={{ border: '2px dashed var(--border-color)', padding: 32, borderRadius: 8, textAlign: 'center', color: 'var(--text-muted)' }}>📁 拖拽文件到此�?�?点击选择�?vibe JSON + 音频�?/div>
      ...
      {activeTab === 'sandbox' && (
        <div style={{ background: 'var(--bg-card)', padding: 24, borderRadius: 12 }}>
          ...
          <span style={{ padding: '6px 14px', borderRadius: 20, fontSize: 12, background: 'var(--badge-green-bg)', color: 'var(--badge-green-text)' }}>BPM 连续�?�?/span>
          <span style={{ marginLeft: 8, padding: '6px 14px', borderRadius: 20, fontSize: 12, background: 'var(--badge-green-bg)', color: 'var(--badge-green-text)' }}>风格一致�?�?/span>
        </div>
      )}
