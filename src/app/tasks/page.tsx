...
interface Task {
  ...
  actualCost: number;
  compensationStatus: 'none' | 'insuring' | 'paid' | 'confirmed' | 'unavailable';
...
const MOCK_TASKS: Task[] = [
  { id: 'task-001', name: '图像分类模型训练', type: '图像识别', urgency: '正常', estimatedCost: 120, actualCost: 118, compensationStatus: 'unavailable', compensationAmount: 0, isManaged: true, status: '已完成', createdAt: '2026-05-09', progress: 100 },
  { id: 'task-002', name: '文本摘要生成', type: '自然语言处理', urgency: '加急', estimatedCost: 80, actualCost: 85, compensationStatus: 'unavailable', compensationAmount: 0, isManaged: false, status: '运行中', createdAt: '2026-05-10', progress: 65 },
  { id: 'task-003', name: '语音识别微调', type: '语音技术', urgency: '限时', estimatedCost: 200, actualCost: 200, compensationStatus: 'unavailable', compensationAmount: 0, isManaged: true, status: '失败', createdAt: '2026-05-08', progress: 42 },
  ...
  { id: 'task-005', name: '自然语言推理', type: '自然语言处理', urgency: '正常', estimatedCost: 95, actualCost: 90, compensationStatus: 'none', compensationAmount: 0, isManaged: true, status: '已完成', createdAt: '2026-05-06', progress: 100 },
];
...
export default function TasksPage() {
  ...
  const filteredTasks = filterStatus === '全部' ? tasks : tasks.filter(t => t.status === filterStatus);
    ...
    setTasks(prev => [newTask, ...prev]);   // ← 正确位置
    setNewTaskName('');
    setNewDuration('');
    setNewManaged(false);
    setShowCreate(false);
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        {['全部', '运行中', '已完成', '失败'].map(s => (
          <button key={s} onClick={() => setFilterStatus(s)} style={{
            ...
            borderRadius: 20, cursor: 'pointer', fontSize: 'var(--spark-font-size-sm)',
            fontWeight: filterStatus === s ? 600 : 400, transition: 'all 0.2s',
          }}>{s}</button>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: 16 }}>
        {filteredTasks.map(task => (
          <TaskCard key={task.id} task={task} onRetry={() => alert('已重新提交')} onViewCompensation={() => setSelectedTask(task)} />
        ))}
      </div>
      <SparkModal isOpen={showCreate} onClose={() => setShowCreate(false)} title="🚀 启动新任务">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <input placeholder="任务名称" value={newTaskName} onChange={e => setNewTaskName(e.target.value)} style={{ width: '100%', padding: '10px 12px', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, fontSize: 14, background: 'rgba(255,255,255,0.06)', color: '#fff', boxSizing: 'border-box' }} />
          <select value={newBizType} onChange={e => setNewBizType(e.target.value)} style={{ width: '100%', padding: '10px 12px', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, fontSize: 14, background: 'rgba(255,255,255,0.06)', color: '#fff', boxSizing: 'border-box' }}>
            {BUSINESS_TYPES.map(t => <option key={t}>{t}</option>)}
          </select>
          <input placeholder="预计耗时（小时）" type="number" value={newDuration} onChange={e => setNewDuration(e.target.value)} style={{ width: '100%', padding: '10px 12px', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, fontSize: 14, background: 'rgba(255,255,255,0.06)', color: '#fff', boxSizing: 'border-box' }} />
