...
export default function ProviderPage() {
  ...
  if (status === 'none') return (
    <div style={{ maxWidth: 500, margin: '40px auto', padding: '0 20px' }}>
      <h1 style={{ fontSize: 24, fontWeight: 'bold', marginBottom: 20 }}>🖥�?节点入驻申请</h1>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <input placeholder="所在机�?/ 地区" value={location} onChange={(e) => setLocation(e.target.value)} style={inputStyle} />
  ...
  return (
    <div style={{ padding: '40px 20px', maxWidth: 900, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, fontWeight: 'bold', marginBottom: 8 }}>🖥�?节点提供者平�?/h1>
      <p style={{ color: 'var(--text-muted)', marginBottom: 24 }}>已签�?· 管理您的算力节点，按任务量获得收益�?/p>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '2px solid var(--border-color)' }}>
        {(['nodes', 'revenue', 'withdraw'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {tab === 'nodes' && '🖥�?我的节点'} {tab === 'revenue' && '💰 收益'} {tab === 'withdraw' && '💳 提现'}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'nodes' && (
        <div style={{ background: 'var(--bg-card)', padding: 24, borderRadius: 12 }}>
          ...
          <p style={{ color: 'var(--text-muted)' }}>当前在线节点：{nodeCount} �?/p>
          <p style={{ color: 'var(--text-muted)' }}>累计完成任务：{taskCount} �?/p>
