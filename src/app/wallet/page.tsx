...
export default function WalletPage() {
  ...
  const handleRecharge = async () => {
    ...
    if (isNaN(val) || val <= 0) { alert('请输入有效金�?); return; }
    try {
  ...
  return (
    <div style={{ padding: '40px 20px', maxWidth: 500, margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--bg-card)', padding: 32, borderRadius: 16, border: '1px solid var(--border-color)', textAlign: 'center', marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 48, fontWeight: 'bold', color: 'var(--btn-primary-bg)', margin: '12px 0' }}>¥{balance.toFixed(2)}</p>
      ...
      {showRecharge && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'var(--bg-card)', padding: 30, borderRadius: 12, width: '90%', maxWidth: 360 }}>
            <h3 style={{ marginBottom: 16 }}>💰 充�?/h3>
            <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="请输入金�? style={{ width: '100%', padding: '10px 12px', border: '1px solid var(--input-border)', borderRadius: 8, fontSize: 14, background: 'var(--input-bg)', color: 'var(--text-primary)', marginBottom: 16 }} />
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleRecharge} style={{ padding: '8px 16px', background: 'var(--btn-success-bg)', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer' }}>确认充�?/button>
            </div>
          </div>
        </div>
      )}
